...
function message(subject: string, paragraphs: string[], action: string, url: string, sms: string, transactional = false): AcademyMessage {
  ...
  return {
    ...
    message_html: paragraphs.map((p) => `<p>${html(p)}</p>`).join("") + `<p><a href="${html(url)}">${html(action)}</a></p><p>${html(footer)}</p>`,
...
export function composeLearningMessage(c: LearningMessageContext): AcademyMessage | null {
  ...
  switch (c.eventName) {
    case "learning_dropoff": {
      ...
      const time = c.resumeSeconds !== null && Number.isFinite(c.resumeSeconds) && c.resumeSeconds >= 0 ? formatTime(c.resumeSeconds) : null;
    ...
    case "learning_practice": {
      ...
      const weak = (c.weakPoints ?? []).map((x) => clean(x, 360)).filter(Boolean).slice(0, 2);
      ...
      next = weak.length ? `Focus on this before your next attempt: ${weak.join(" ")} Then explain it using your own business and retake the check.` : `Revisit the lesson notes for “${clean(c.lesson.skill, 100)},” explain the idea using your own business, then retake the check. Ask ${c.assistant} for a smaller example if you get stuck.`;
    ...
    case "learning_feedback": {
      ...
      next = feedback ? `Instructor feedback: “${feedback}” Update the relevant section, then submit it again. You can ask ${c.assistant} to help you work through the feedback.` : `Open your activity sheet, read your instructor’s feedback, and revise the relevant section before submitting it again.`;
    ...
    case "learning_stalled":
      ...
      next = c.missingActivity ? `Start with “${clean(c.missingActivity.label, 100)}”: ${clean(c.missingActivity.hint, 260)} Add a specific response, then save it. You can do the next section later.` : "Your activity fields have saved responses. Review your answers and submit the sheet when you are ready for instructor feedback.";
...
export function composeWelcomeMessage(paid: boolean, assistant: "Thoth" | "AI Spin", freeVideoReady: boolean) {
  return message(
...
export function composeWebinarReminder() {
  return message(
    ...
    ["You registered for the free training and have not started the recording in this account yet.", "Set aside 10 minutes. Name one repeated business task you would like to improve, then open the training. Thoth can help you turn that idea into a clear first step."],
...
export function composeAccessCodeMessage(tier: string, code: string, expiresAt: string, emailTicket = false, programmeEndsAt?: string | null) {
  const names: Record<string, string> = { ga: "General Admission", vip: "VIP", vault: "Emerald / Vault", accelerator: "Accelerator" };
  const label = names[tier] ?? "Course";
  const cutoff = programmeEndsAt ? ` This programme ends at ${new Date(programmeEndsAt).toISOString()}; activating later does not extend that date.` : "";
  if (emailTicket) return message(
    ...
    "Activate my purchased lessons", "https://aiautopilotsummit.com/redeem",
    `AI AutoPilot: Your ${label} purchase is verified. Sign in with your purchase email, then activate your purchased lessons when ready.`, true,
  );
  return message(
    ...
    [`Your ${label} purchase is verified. Sign in or create your account using this purchase email, then enter your access code.`,
      `Your access code: ${code}`, `Redeem this code before ${new Date(expiresAt).toISOString()}. Your course access period starts when you redeem it.${cutoff}`],
