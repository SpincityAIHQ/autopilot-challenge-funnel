...
6. The tutor now calls the managed Lovable AI Gateway (`https://ai.gateway.lovable.dev/v1/chat/completions`) with the platform-provisioned server-side `LOVABLE_API_KEY`, default model `google/gemini-3.7-flash` (override with `ACADEMY_TUTOR_MODEL`). No user-supplied AI key is required. Remaining owner action: set `ACADEMY_TUTOR_ENABLED=true` (`RATE_LIMIT_HMAC_SECRET` is already configured) and test grounded responses. Optional `ACADEMY_TUTOR_DAILY_LIMIT` (100 requests/day default) plus the 15/hour per-learner and 60/hour network caps remain; these are request caps, not a dollar-denominated spend guarantee. Learners see which AI service receives their question and lesson work before consenting. Credit or policy denials from the gateway surface as an honest paused state; no simulated answers.
