...
6. Configure the chosen tutor model/provider and test grounded responses. Current adapter uses OpenAI Responses with `OPENAI_API_KEY`, `ACADEMY_TUTOR_MODEL`, `ACADEMY_TUTOR_ENABLED=true`, `RATE_LIMIT_HMAC_SECRET`, and optional `ACADEMY_TUTOR_DAILY_LIMIT` (100 requests/day default). This is not a dollar-denominated spend guarantee. Managed Lovable AI may replace the provider adapter while preserving the data/access boundaries.
