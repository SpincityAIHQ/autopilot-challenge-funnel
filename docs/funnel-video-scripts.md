# AI AutoPilot 2-Day Summit — Final Funnel Video Scripts

## Locked schedule

- Day 1: Saturday, August 29, 2026 · 1:00–4:00 PM Eastern
- Day 2: Sunday, August 30, 2026 · 1:00–4:00 PM Eastern
- Room opens: 12:45 PM Eastern both days
- VIP Build Lab: Sunday, August 30, 2026 · 4:15–5:45 PM Eastern, immediately after Day 2

Use plain language. Read naturally. Do not rush. Do not promise income.

---

## 1. Landing-page VSL

**Video slot:** `VITE_SUMMIT_VIDEO_HERO`

Peace Family. I’m Sebastian “SpinCity” Spencer, founder of SpinCityHQ and NuAmenti.

My background is in technology, branding, design, marketing, sales, automation, business systems, and human learning.

For years, I have helped people turn ideas into brands, offers, pages, campaigns, apps, and working systems.

When AI entered the picture, I saw something bigger than content creation or writing prompts.

I saw a new kind of business.

A business where AI can research the market, study the customer, run the numbers, help build internal apps, create marketing, prepare sales work, follow up, check results, and improve the next run.

That is the work behind SpinCityHQ and NuAmenti.

SpinCityHQ helps people see and seize the AI opportunity.

NuAmenti turns that opportunity into a working business system.

That is why we created the AI AutoPilot 2-Day Summit.

This is not an event where you sit for two days, hear a list of tools, and still do not know what to build.

This is a working Summit.

Day 1 is Saturday, August 29, from 1:00 to 4:00 PM Eastern.

We build the business foundation.

You will choose the niche you want to serve. You will define the problem you want to solve. You will map the customer, the offer, the work, and the numbers.

You will create your Business Infrastructure Map so you can see the pages, apps, data, tools, people, and handoffs your business needs.

You will plan an internal business app.

And you will set up your AI Business GPS.

Your AI Business GPS tells your tools who you serve, what you sell, how the business works, what rules to follow, what numbers matter, and what should happen next.

Day 2 is Sunday, August 30, from 1:00 to 4:00 PM Eastern.

We hire the AI team.

You will map the agents your business needs. You will create their jobs, goals, rules, tools, and reporting structure.

You will learn where to hire, build, or connect AI agents that can work at a high level.

Then we connect those agents into workflows and loops.

A workflow moves work from one step to the next.

A loop checks what happened, learns from the result, and improves the next run.

We will connect the system to marketing, follow-up, sales, and monetization.

This Summit is for business owners, creators, consultants, service providers, experts, and founders who have real knowledge or a strong idea but need a better system around it.

You do not need to be a coder.

You do need to bring a real business, skill, offer, or idea that you are ready to work on.

This is not for people looking for one magic prompt.

It is not for people who want AI to make money while they do nothing.

There is no magic button here.

We are building real infrastructure.

You will leave with a Niche and Offer Map, a Business Infrastructure Map, an AI Business GPS, an AI Agent Team Chart, a starter internal app plan, a workflow and loop plan, and a clear 30-day build order.

The room opens at 12:45 PM Eastern both days.

The goal is simple.

Build the business.

Hire the AI team.

Put the repeat work on autopilot.

Keep human control over the decisions that matter.

Click Reserve Your Seat and complete your registration.

Peace Family. I will see you inside.

---

## 2. General Admission checkout video

**Video slot:** `VITE_SUMMIT_VIDEO_CHECKOUT`

Peace Family. You made it to the final step to reserve your seat.

The Summit is live Saturday, August 29 and Sunday, August 30 from 1:00 to 4:00 PM Eastern. The room opens at 12:45 both days.

Your General Admission ticket gives you both live days.

On Day 1, we build your niche, offer, business numbers, infrastructure map, internal app plan, and AI Business GPS.

On Day 2, we structure your AI agent team, workflows, loops, marketing, follow-up, and monetization.

This is not a tool show. You are coming to work on a real business, skill, offer, or idea.

Use an email address you check often. Your receipt, access information, calendar links, and event reminders will go there.

Review the details below, complete the secure checkout, and we will welcome you on the next page.

I appreciate you, Family. Let’s build.

---

## 3. GA purchase video that introduces VIP

**Video slot:** `VITE_SUMMIT_VIDEO_THANK_YOU_GA`

Peace Family. Thank you.

Your General Admission seat is confirmed.

You are joining us Saturday, August 29 and Sunday, August 30 from 1:00 to 4:00 PM Eastern. The room opens at 12:45 both days.

Watch for your NuAmenti welcome email, calendar links, preparation steps, private room link, and the resources connected to your ticket.

Before Day 1, gather one business, skill, offer, or idea you want to work on.

Also bring a list of the tasks that take too much time, your current prices and goals, your basic business numbers, and any logo or brand files you have.

Your General Admission ticket is complete by itself.

You may also see the optional VIP Implementation Experience below.

VIP is for people who want 30-day recordings, priority questions, deeper agent and workflow tools, and the live VIP Build Lab.

The VIP Build Lab starts Sunday at 4:15 PM Eastern, only 15 minutes after Day 2 ends. You stay in the flow and keep building while everything is fresh.

Your General Admission seat stays active whether you add VIP or not.

Thank you again for trusting SpinCityHQ and NuAmenti.

---

## 4. VIP offer video

**Video slot:** `VITE_SUMMIT_VIDEO_VIP_OFFER`

Peace Family.

Your General Admission ticket is already secure.

VIP is the next step for people who want more access, more support, and more time to apply the work.

The main Summit ends Sunday at 4:00 PM Eastern.

The VIP Build Lab starts at 4:15 PM Eastern and runs until 5:45 PM.

That means you do not wait days for the energy to cool off.

You take a short break, stay with us, and go deeper while the work is still fresh.

VIP adds 30-day access to the Summit recordings.

It adds priority questions.

It adds the AI Agent Hiring and Workflow Kit.

And it adds the live Build Lab where we work through real agent roles, app plans, workflows, loops, handoffs, and approval rules.

VIP is an add-on. It does not replace General Admission.

Your GA ticket stays active if you pass.

When this level of support makes sense for you, add VIP below.

---

## 5. VIP purchase video that introduces the Vault

**Video slot:** `VITE_SUMMIT_VIDEO_THANK_YOU_VIP`

Peace Family. Thank you for adding the VIP Implementation Experience.

Your VIP access is confirmed.

You now have both live Summit days, 30-day recordings, priority question access, the AI Agent Hiring and Workflow Kit, and the VIP Build Lab.

The Lab begins Sunday, August 30 at 4:15 PM Eastern, right after the main Summit ends.

The next option on this page is the AI AutoPilot Implementation Vault.

VIP gives you deeper access and live support.

The Vault gives you reusable operating assets.

Inside the Vault, you receive editable maps, agent job sheets, internal app plans, workflow templates, loop builders, business math tools, marketing systems, and implementation guides.

The goal is to stop rebuilding every system from a blank page.

The Vault does not replace VIP.

Your VIP access stays active whether you add the Vault or not.

Review what is included below and choose the level that fits the way you want to build.

---

## 6. Vault purchase video that introduces the private Intensive

**Video slot:** `VITE_SUMMIT_VIDEO_THANK_YOU_VAULT`

Peace Family. Thank you for adding the AI AutoPilot Implementation Vault.

Your Vault access is confirmed.

You now have the maps, agent job sheets, app-planning tools, workflow templates, business math resources, marketing systems, and implementation guides designed to help you keep building.

The Vault gives you the operating assets.

The final optional step is a private Strategy and Build Intensive with me.

This is not another general coaching call.

We focus on the one working piece that can create the most movement in your business.

That may be your niche and offer, your AI Business GPS, your internal app, your AI agent team, a sales or marketing workflow, a follow-up loop, or the money path connecting the system.

We diagnose the bottleneck, choose the right next build, and work through it together.

There are only ten private sessions because I am personally involved in each one.

The seat count shown on this page is the real count.

Your Summit, VIP, and Vault access stay active whether you book the private session or not.

Review the details below and reserve a seat only when that level of support fits your business.

---

# Final exit-confirmation videos

These videos do not sell another offer. They confirm what the buyer owns and explain what happens next.

## 7A. GA-only final confirmation

**Video slot:** `VITE_SUMMIT_VIDEO_EXIT_GA`

Peace Family. You are all set with General Admission.

Your ticket is complete.

You are joining us Saturday, August 29 and Sunday, August 30 from 1:00 to 4:00 PM Eastern. The room opens at 12:45 both days.

Watch for your NuAmenti email with the calendar links, private room link, preparation steps, and the core resources connected to General Admission.

Bring one real business, skill, offer, or idea. Bring the tasks that take too much time and your basic business numbers.

Thank you for trusting SpinCityHQ and NuAmenti. I will see you inside.

## 7B. VIP-only final confirmation

**Video slot:** `VITE_SUMMIT_VIDEO_EXIT_VIP`

Peace Family. You are all set with General Admission and VIP.

You have both live Summit days, 30-day recordings, priority questions, the AI Agent Hiring and Workflow Kit, and the VIP Build Lab.

The VIP Lab starts Sunday, August 30 at 4:15 PM Eastern, 15 minutes after Day 2 ends, and runs until 5:45 PM.

Watch your NuAmenti email for your calendar links, private room link, VIP access, and preparation steps.

Thank you, Family. I will see you at the Summit and in the VIP Lab.

## 7C. Vault final confirmation

**Video slot:** `VITE_SUMMIT_VIDEO_EXIT_VAULT`

Peace Family. You are all set with the Summit, VIP, and the AI AutoPilot Implementation Vault.

You have both live days, the VIP Build Lab right after Day 2, the recordings, priority questions, deeper implementation support, and the reusable Vault assets.

Your NuAmenti email will include the private event links and the secure route to your maps, agent sheets, app plans, workflows, loops, and marketing tools.

Thank you for trusting us with the full implementation path.

## 7D. Full-path Intensive confirmation

**Video slot:** `VITE_SUMMIT_VIDEO_EXIT_INTENSIVE`

Peace Family. Thank you for completing the full AI AutoPilot Summit path.

You now have General Admission, VIP, the Implementation Vault, and the private Strategy and Build Intensive.

There is no additional offer on this page.

Our team will email you from Info@NuAmenti.com with the scheduling link for your private session.

Before the session, gather the goals, offers, numbers, apps, tools, and current process connected to the system you want to build. Do not send passwords or private client information.

Watch your inbox for the Summit links, preparation emails, resource access, and private-session instructions.

Thank you for trusting me, SpinCityHQ, and NuAmenti.

Welcome to the Family.

---

# Live stage scripts

## Day 2 VIP close — 3:45 PM Eastern

Family, the main Summit ends at 4:00 PM.

Your General Admission ticket is complete. You have the maps, the business foundation, the agent-team plan, and the 30-day build order.

For the people who want to keep going while this is still fresh, the VIP Build Lab starts at 4:15 PM Eastern.

VIP gives you 30-day recordings, priority questions, the AI Agent Hiring and Workflow Kit, and the live Lab from 4:15 to 5:45.

We will take a 15-minute break. The VIP link is in the chat, your email, and your text messages if you opted in.

Add VIP if you want to stay and build deeper.

If you are complete with General Admission, thank you. Use your next-steps page and follow the 30-day order.

## VIP Lab Vault close — about 5:25 PM Eastern

Family, VIP gave you the deeper room, recordings, questions, and implementation time.

The next option is for the person who wants the reusable operating library.

The AI AutoPilot Implementation Vault includes the editable maps, agent job sheets, app plans, workflow templates, loop builders, business math tools, marketing systems, and implementation guides.

VIP is still complete without it.

The Vault is for the person who does not want to rebuild the system from a blank page.

The link is in the VIP chat. When you add the Vault, the next page will also show the limited private Strategy and Build Intensive.
